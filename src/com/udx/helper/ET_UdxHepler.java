...
/**
 * Dr.Lu's Fvcom,step4,[output] [cha_ET.txt2] conversion Tool(未完成， 因转换数据很大，并且转换之后精度可能丢失，只转换了第一组数据)
 * @author CK
 *
 */
public class ET_UdxHepler {
	
public static UdxDataset ET_ToUdx(String fileFullPath,String dllPaths)throws Exception{
	   ...
	   //////////////////////////////////////////////////////////////////////////////////
		...
		int oneLineDataCount=1;
		byte[] oneData=new byte[8];	
		int dataGroupCount=0;
		int oneLineDataIndex=0;
		int dataIndex=0;
		Double[] oneDoubleData = new Double[1];
		int End=-1;
		//创建xml
		...
		UdxHandlUtil.createNode(headNode, UdxDataType.UdxKernelIntValue, "oneGroupDataCount", String.valueOf(pointsCount));
		...
		do{			
			...
			do{
				...
				if(End!=-1){
					oneDoubleData[oneLineDataIndex++]=DataUtil.bytes2Double(oneData);											
					if(oneLineDataIndex==oneLineDataCount){
						dataIndex+=1;
						String dataStr=String.valueOf(oneDoubleData[0]);
						UdxHandlUtil.createNode(dataGroupNode, UdxDataType.UdxKernelRealValue, String.valueOf(dataIndex), dataStr);					
						oneLineDataIndex=0;
						if(dataIndex==pointsCount){
							dataIndex=0;
							dataGroupCount+=1;
...
public static boolean ET_ToUdx(String inputPath,String dllPaths,String SavePath)throws Exception{
	   
	UdxDataset dataset=ET_ToUdx(inputPath,dllPaths);
	 if(SavePath=="TestPath"){
		   SavePath="D:\\Workspace\\FvCom\\tmpData\\output\\cha_ET.xml";
...
public static String UdxToET(UdxDataset dataset) throws Exception{
	   String dllPath="E:\\DEVELOPERKIT\\Work\\JWorkSpace\\workspace2_cpp_java\\UdxHelper\\Library";
		UdxDataset udx_return_Dt = new UdxDataset(dllPath);
		String fileName="D:\\Workspace\\FvCom\\tmpData\\output\\cha_ET.xml";
...
public static boolean UdxToET(UdxDataset dataset,String SavePath) throws Exception{
	   String DataStr =UdxToET(dataset);
	   if(DataStr.equals("")||DataStr==null){
		   System.out.println("conversion failed.");
		   return false;
	   }
	   if(SavePath=="TestPath"){
		   SavePath="D:\\Workspace\\FvCom\\tmpData\\output\\cha_ET1.txt2";
	   }else if(SavePath.equals("")||SavePath==null){
		   System.out.println("the SavePath is not right.");
		   return false;
	   }
